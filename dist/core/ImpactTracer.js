...
import { SymbolAnalyzer } from './SymbolAnalyzer.js';
import { PropagationTracker } from './PropagationTracker.js';
export class ImpactTracer {
    analyzer;
    propagationTracker;
    projectRoot;
    config;
    constructor(projectRoot, tsConfigPath, config) {
     ...
     * 追踪改动影响
     */
    async traceImpact(file, symbol, changeType = 'modify') {
        // 规范化路径
        ...
        const analysis = this.analyzer.analyzeImpact(targetSymbol || '', changeType === 'add' ? 'modify' : changeType, targetSymbol ? undefined : normalizedFile // 有符号名时全局搜索
        );
        // 构建受影响文件列表
    ...
    inferMainSymbol(filePath) {
        ...
        if (symbol && symbol.exports.length > 0)
            return symbol;
     ...
     * 分类受影响的文件
     */
    categorizeAffectedFiles(references) {
     ...
     * 将传播节点转换为传播路径
     */
    convertPropagationNodesToPaths(nodes, symbolKind) {
        const paths = [];
        // 添加根节点作为路径
        for (const node of nodes) {
            ...
            // 递归处理子节点
            const traverseChildren = (children, parentSymbol) => {
     ...
     * 构建传播路径
     */
    buildPropagationPaths(analysis) {
        const paths = [];
        // 从调用图构建路径
     ...
     * 生成建议
     */
    generateRecommendations(symbol, analysis, changeType) {
        const recs = [];
        // 基于风险等级
