...
export class PropagationTracker {
   ...
   * 追踪符号的传播路径
   */
  trace(symbolName: string, filePath: string, maxDepth: number = 5): PropagationNode[] {
    const roots: PropagationNode[] = [];
   ...
   */
  private findNestedProperty(propertyName: string, filePath: string): {
    definition: { node: Node; type: 'property' };
    parentObj: string;
    ...
    // 查找所有对象字面量
    const references: Array<{ file: string; line: number; context: string; parentObj: string }> = [];
    ...
    // 返回第一个找到的父对象名
    return {
      definition: { node: references[0] as any, type: 'object' as any },
   ...
   * 查找对象字面量中属性的父对象名
   */
  private findParentObjectName(node: Node): string | null {
   ...
   * 查找符号定义
   */
  private findDefinition(symbolName: string, filePath: string): {
    node: Node;
    type: 'constant' | 'variable' | 'type' | 'object' | 'enum' | 'nested';
    value?: string;
  } | null {
    const sourceFile = this.project.getSourceFile(filePath);
    if (!sourceFile) return null;

    let result: { node: Node; type: 'constant' | 'variable' | 'type' | 'object' | 'enum' | 'nested'; value?: string } | null = null;
   ...
   * 查找符号的所有引用位置
   */
  private findReferences(symbolName: string, filePath: string): Array<{
    node: Node;
    file: string;
    line: number;
    parent: Node;
      ...
      sourceFile.forEachDescendant(node => {
        if (Node.isIdentifier(node)) {
          if (node.getText() === symbolName) {
            ...
            if (parent && Node.isVariableDeclaration(parent) && (parent as VariableDeclaration).getName() === symbolName) {
              return;
            }
   ...
   * 获取上下文代码
   */
  private getContextCode(node: Node): string {
  ...
  private buildPropagationTree(
    ...
    for (const ref of refs) {
      ...
      const childNode: PropagationNode = {
        symbol: symbolName,
        file: ref.file,
        line: ref.line,
   ...
   * 分类符号的使用方式
   */
  private classifyUsage(node: Node, parent: Node): 'constant' | 'variable' | 'type' | 'object' | 'property' | 'functionArg' | 'return' {
   ...
   * 获取赋值的变量名
   */
  private getAssignedVariable(node: Node, parent: Node): string | null {
    if (Node.isBinaryExpression(parent)) {
   ...
   * 获取包含函数名
   */
  private getContainingFunction(node: Node): string | null {
    ...
    while (current) {
      ...
      if (Node.isArrowFunction(current)) {
        ...
        if (Node.isPropertyAssignment(parent)) {
          return (parent as PropertyAssignment).getName() ?? 'arrow';
        }
        return 'arrow';
      }
